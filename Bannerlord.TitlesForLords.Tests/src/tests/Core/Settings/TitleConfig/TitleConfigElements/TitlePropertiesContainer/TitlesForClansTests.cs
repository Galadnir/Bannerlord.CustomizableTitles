...
namespace Bannerlord.TitlesForLords.Tests.src.tests.Core.Settings.TitleConfig.TitleConfigElements.TitlePropertiesContainer {
	[TestClass]
	public class TitlesForClansTests {

		static readonly TitlesForClans FullProperties = JsonConvert.DeserializeObject<TitlesForClans>(JsonObjectsForTests.FullTitlesForClansProperties().ToString())!;
		static readonly TitlesForClans EmptyProperties = JsonConvert.DeserializeObject<TitlesForClans>(JsonObjectsForTests.EmptyProperties().ToString())!;
